  ...
  /* Infinite loop */
	for(;;)
	{
		...
		if(g_OLEDEnable)
		{
			...
			OLED_ShowChar(1,4,':');
			OLED_ShowNumNoLength(1,5,cnt++);
			g_OLEDEnable = 1;
		}
		vTaskDelay(pdMS_TO_TICKS(100));
		
		t2 = system_get_ns()/1000;
		OLED_ClearLine(2);
		OLED_ShowString(2,1,"t2-t1:");
		OLED_ShowNumNoLength(2,7,t2-t1);
		
		vTaskDelayUntil(&pretime,pdMS_TO_TICKS(500));
		
		t3 = system_get_ns()/1000;
		OLED_ClearLine(3);
		OLED_ShowString(3,1,"t3-t1:");
		OLED_ShowNumNoLength(3,7,t3-t1);
	}
