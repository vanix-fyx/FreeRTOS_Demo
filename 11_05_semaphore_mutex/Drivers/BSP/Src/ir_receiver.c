...
#if IRReceiverTaskFunction_ENABLE
/* 任务句柄 */
...
{
	return g_xQueueIRReceiver_Handle;
}
...
//	PutKeyToBuf(datas[2]);
	...
	i_data.dev = datas[0];
	i_data.data = datas[2];
	xQueueSendToBackFromISR(g_xQueueIRReceiver_Handle,&i_data,NULL);
...
#endif
		g_xQueueIRReceiver_Handle = xQueueCreate(IRReceverQueueLength,sizeof(IRReceiver_Data_Struct));
}
...
{
	if(pdPASS == xQueueReceive(g_xQueueIRReceiver_Handle,rdata,0 ))//portMAX_DELAY
		return 0;
	else 
		return -1;
}
