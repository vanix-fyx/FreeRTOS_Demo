...
QueueHandle_t g_xQueueIRReceiverHandle;
IRReceiver_Data_Struct IRReceiver_Data; 
...
//	PutKeyToBuf(datas[2]);
	...
	i_data.dev = datas[0];
	i_data.data = datas[2];
	xQueueSendToBackFromISR(g_xQueueIRReceiverHandle,&i_data,NULL);
...
#endif
		g_xQueueIRReceiverHandle = xQueueCreate(10,sizeof(IRReceiver_Data_Struct));
}
...
{
	if(pdPASS == xQueueReceive(g_xQueueIRReceiverHandle,rdata,100 ))//portMAX_DELAY
		return 0;
	else 
		return -1;
}
...
void vIRReceiverTaskFunction(void* pvParameters)
{
    /* PB10在MX_GPIO_Init()中已经被配置为双边沿触发, 并使能了中断 */
		...
		/* IRReceiver_IRQ_Callback();需要把这个函数放在中断函数EXTI15_10_IRQHandler()里*/
    uint32_t lastdata;
		uint8_t platform_idata;
		for(;;)
		{
        // 1. 读取红外数据
				if(IRReceiver_Read(&IRReceiver_Data) == 0)
        {
            // 2. 重复码处理：先恢复上次有效按键
            if(IRReceiver_Data.data == 0x00 && lastdata != 0)
            {
                IRReceiver_Data.data = lastdata; // 重复码复用上次按键
            }
						
            // 2. 根据按键值更新坐标
						switch(IRReceiver_Data.data)
						{
//							case 0x00:	// 重复码
//													IRReceiver_Data.data = lastdata;
//													break;
							case 0xe0:	// 左移
													platform_idata = UPT_MOVE_LEFT;
													xQueueSend(g_xQueuePlatformHandle,&platform_idata,0);
													break;
							case 0x90:	// 右移
