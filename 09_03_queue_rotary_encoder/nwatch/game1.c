...
/* 挡球板队列 */
QueueHandle_t g_xQueuePlatformHandle;
...
void Platform_task(void *params)
{
    ...
    {
				/* 读挡球板队列 */
				if(pdPASS == xQueueReceive(g_xQueuePlatformHandle,&g_platformMoveDir,portMAX_DELAY))
...
{	
    draw_init();
		game_init_objects();
		g_xQueuePlatformHandle = xQueueCreate(10,sizeof(uint8_t));
