...
/* 队列集输入队列 & 队列集元素 */
static QueueSetHandle_t g_xQueueSetInput_Handle;	//输入队列集
static QueueHandle_t g_xQueueIRReceiver_Handle;	//红外队列
static QueueHandle_t g_xQueueRotary_Handle;			//旋转编码器队列
/* 挡球板队列 */
...
void IRReceiverProcess(void)
{
		...
		// 1. 读取红外数据
		if(pdPASS == xQueueReceive(g_xQueueIRReceiver_Handle,&rdata,0))
		{				
				// 2. 根据按键值更新坐标
				switch(rdata.data)
				{
...
void ProcessRotaryData(void)
{
		...
		/* 读旋转编码器队列 */
		if(pdPASS == xQueueReceive(g_xQueueRotary_Handle,&rdata,0))
		{
			/* 处理数据 */
			if(rdata.speed > 0)
			{
...
void Input_task(void *params)
{
    ...
    {
			/* 读取队列集，获得队列句柄 */
			xQueueHandle = xQueueSelectFromSet(g_xQueueSetInput_Handle,portMAX_DELAY);
			
			/* 读取队列，获得队列数据 */
			if(xQueueHandle)
			{
				if(xQueueHandle == g_xQueueIRReceiver_Handle)
				{
					/* 处理红外数据，写入挡球板队列 */
					IRReceiverProcess();
				}
				else if(xQueueHandle == g_xQueueRotary_Handle)
				{
					/* 处理旋转编码器数据，写入挡球板队列 */
					ProcessRotaryData();
				}
...
{	
    draw_init();
		...
		g_xQueueSetInput_Handle = xQueueCreateSet(IRReceverQueueLength + RotaryQueueLength);
		g_xQueueIRReceiver_Handle = GetQueueIR();
		g_xQueueRotary_Handle = GetQueueRotary();
		
		xQueueAddToSet(g_xQueueIRReceiver_Handle,g_xQueueSetInput_Handle);
		xQueueAddToSet(g_xQueueRotary_Handle,g_xQueueSetInput_Handle);
