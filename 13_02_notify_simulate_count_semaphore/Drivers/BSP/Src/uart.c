...
#if UARTTaskFunction_ENABLE
/* 任务句柄 */
StaticTask_t xUARTTaskTCB;	//Uart静态任务控制块（TCB）
StackType_t xUARTTaskStack[128];	//任务栈空间数组
TaskHandle_t xUARTTaskHandle = NULL;	// Uart任务句柄
#endif
...
#if UARTTaskFunction_ENABLE	
		/*创建任务：IRReceiver*/
		xUARTTaskHandle = xTaskCreateStatic(vUARTTaskFunction,"UartTask",128,NULL,osPriorityNormal,
...
void vUARTTaskFunction(void* pvParameters)
{
	while(1)
	{
