...
#if 0	//关闭的任务 - 句柄与变量定义
/* 静态任务 - 句柄与变量定义 ---------------------------------------------------------*/
	StaticTask_t xLEDTaskTCB;	//LED静态任务控制块（TCB）
	StackType_t xLEDTaskStack[128];	//任务栈空间数组
	TaskHandle_t xLEDTask_Handle = NULL;	// LED任务句柄
	...
	TaskHandle_t xIRReceiverTask_Handle = NULL;	// IRReceiver任务句柄

/* 动态任务 - 句柄与变量定义 ---------------------------------------------------------*/
/* 使用同一函数创建不同任务OLEDPrint ---------------------------------------------------------*/
	TaskHandle_t xOLEDPrintTask1_Handle = NULL;
	TaskHandle_t xOLEDPrintTask2_Handle = NULL;
	TaskHandle_t xOLEDPrintTask3_Handle = NULL;
...
//  defaultTaskHandle = osThreadNew(StartDefaultTask, NULL, &defaultTask_attributes);
  ...
  /*创建任务：Game1*/
	...
	//静态任务
  /*创建任务：LED*/
  xLEDTask_Handle = xTaskCreateStatic(vLEDTaskFunction,"LEDTask",128,NULL,osPriorityNormal,
  ...
  /*创建任务：IRReceiver*/
  xIRReceiverTask_Handle = xTaskCreateStatic(vIRReceiverTaskFunction,"IRReceiverTask",256,NULL,osPriorityNormal,
		  xIRReceiverTaskStack,&xIRReceiverTaskTCB);

  /*创建任务：旋转编码器*/
  xRotaryTask_Handle = xTaskCreateStatic(vRotaryEncoderTaskFunction,"RotaryEncoderTask",256,NULL,osPriorityNormal,
		  xRotaryTaskStack,&xRotaryTaskTCB);

	//动态任务
  /*使用同一函数创建不同任务OLEDPrint*/
	xTaskCreate(vOLEDPrintTaskFuction,"OLEDPrintTask1",128,
							&g_Task1Info,osPriorityNormal,&xOLEDPrintTask1_Handle);
	xTaskCreate(vOLEDPrintTaskFuction,"OLEDPrintTask2",128,
							&g_Task2Info,osPriorityNormal,&xOLEDPrintTask2_Handle);
	xTaskCreate(vOLEDPrintTaskFuction,"OLEDPrintTask3",128,
							&g_Task3Info,osPriorityNormal,&xOLEDPrintTask3_Handle);
