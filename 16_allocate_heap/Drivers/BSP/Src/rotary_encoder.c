...
#if RotaryTaskFunction_ENABLE
/* 任务句柄 */
StaticTask_t xRotaryTaskTCB;	//旋转编码器静态任务控制块（TCB）
StackType_t xRotaryTaskStack[256];	//任务栈空间数组
TaskHandle_t xRotaryTask_Handle = NULL;	// 旋转编码器任务句柄
#endif

/* 队列 */
static QueueHandle_t g_xQueueRotary_Handle;
...
//    mdelay(2);
    ...
    pre_time = time;
		...
		rdata.cnt = g_count;
		rdata.speed = g_speed;
		xQueueSendFromISR(g_xQueueRotary_Handle,&rdata,NULL);
}
...
void RotaryEncoder_Init(void)
{
    ...
    /* PB12在MX_GPIO_Init中被配置为中断引脚,上升沿触发 */
		...
		g_xQueueRotary_Handle = xQueueCreate(RotaryQueueLength,sizeof(Rotary_Data));
}
