...
static platformDir g_platformMoveDir = PLATFORM_STOP; // 挡板当前移动意图
...
void game_init_objects(void) 
{
    ...
    g_score = 0;
		...
		velX = 0.5;      // 小球水平速度
		velY = 0.5;      // 小球垂直速度
		g_platformMoveDir = PLATFORM_STOP;// 挡板当前移动意图
}
//挡球板任务
...
//        if (IRReceiver_Read(&dev, &data) == 0)
				if(pdPASS == xQueueReceive(g_xQueueIRReceiver,&IRReceiver_Data,portMAX_DELAY ))
        {
            // 2. 根据按键值更新坐标
            if (IRReceiver_Data.data == 0x00) // 重复码
            {
							if (currentState == STATE_PLAYING)
							{
                IRReceiver_Data.data = IRReceiver_Data.lastdata;
							}
            }
            if (IRReceiver_Data.data == 0xe0) // 左移
            { 
							if (currentState == STATE_PLAYING)
							{
                g_platformMoveDir = PLATFORM_LEFT;
							}
            } 
            if (IRReceiver_Data.data == 0x90) // 右移
            {
							if (currentState == STATE_PLAYING)
							{
                g_platformMoveDir = PLATFORM_RIGHT;
							}
            }
						IRReceiver_Data.lastdata = IRReceiver_Data.data;
						
            if (IRReceiver_Data.data == 0xa8) // 暂停Play
						{
							if(currentState == STATE_PLAYING)
							{
								IRReceiver_Data.data = 0;
								IRReceiver_Data.lastdata = 0;
								g_platformMoveDir = PLATFORM_STOP;
            ...
            if (IRReceiver_Data.data == 0x22) // 重置Test
						{
							currentState = STATE_PLAYING;
							draw_init();
							game_init_objects();
						...
						if (currentState == STATE_PLAYING)
						{
							if(g_platformMoveDir == PLATFORM_LEFT)
							{
                if (platformX > 8) platformX -= 8;
								else platformX = 0;
							}
							else if(g_platformMoveDir == PLATFORM_RIGHT)
							{
