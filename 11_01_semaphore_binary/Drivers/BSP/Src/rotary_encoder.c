...
/* 任务句柄 */
StaticTask_t xRotaryEncoderTaskTCB;	//旋转编码器静态任务控制块（TCB）
StackType_t xRotaryEncoderTaskStack[256];	//任务栈空间数组
TaskHandle_t xRotaryEncoderTaskHandle = NULL;	// 旋转编码器任务句柄
#endif
...
#if RotaryTaskFunction_ENABLE
		/*创建任务：旋转编码器*/
		xRotaryEncoderTaskHandle = xTaskCreateStatic(vRotaryEncoderTaskFunction,"RotaryEncoderTask",256,NULL,osPriorityNormal,
				xRotaryEncoderTaskStack,&xRotaryEncoderTaskTCB);
#endif
	
		g_xQueueRotary = xQueueCreate(RotaryQueueLength,sizeof(Rotary_Data));
}
