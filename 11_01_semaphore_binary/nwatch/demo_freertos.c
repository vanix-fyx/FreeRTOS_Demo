...
/* 信号量 */
static SemaphoreHandle_t	g_xSemaphoreBinaryHandle;
...
void start_task(void* pvParameters)
{
	while(1)
	{
		g_xSemaphoreBinaryHandle = xSemaphoreCreateBinary();
...
void task1(void* pvParameters)
{
	...
	while(1)
	{
		if(pdPASS == xQueueReceive(g_xQueueIRReceiver,&rdata,portMAX_DELAY))
		{
			switch(rdata.data)
			{
				case IR_KEY_1:	xSemaphoreGive(g_xSemaphoreBinaryHandle);
												break;
			}
		}
...
void task2(void* pvParameters)
{
	...
	while(1)
	{
		xSemaphoreTake(g_xSemaphoreBinaryHandle,portMAX_DELAY);
